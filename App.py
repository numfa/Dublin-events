...
if fetch:
    with st.spinner("Ticketmaster veriler cekiliyor..."):
        try:
            events = fetch_events()
    ...
    for i, ev in enumerate(filtered):
        ...
        with cols[i % 3]:
            ...
            st.markdown(
                ...
                + emoji + " " + seg + "</span>"
                + ("<span style='float:right;color:#00e5a0;font-size:12px'>" + price + "</span>" if price else "")
                + "<div style='font-size:15px;font-weight:700;color:#fff;margin:8px 0 4px'>" + ev.get("name", "") + "</div>"
                + "<div style='color:#5b7fff;font-size:11px'>📅 " + date + "</div>"
                + "<div style='color:#4a4e6a;font-size:11px;margin-top:3px'>📍 " + venue_name + "</div>"
                + ("<div style='margin-top:10px'><a href='" + link + "' target='_blank' style='color:" + color + ";font-size:11px'>Bilet Al</a></div>" if link else "")
                + "</div>",
                unsafe_allow_html=True
            )
else:
    st.markdown("<div style='text-align:center;padding:60px 0'><div style='font-size:48px'>🏙️</div><div>Butona bas, etkinlikleri kesfet</div></div>", unsafe_allow_html=True)

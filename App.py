...
def fetch_eventbrite():
    try:
        ...
        for ev in events:
            ...
            venue = ""
            if venue_obj:
                venue = venue_obj.get("name", "") or venue_obj.get("address", {}).get("localized_address_display", "Dublin")
            cat = ev.get("category_id", "")
            img_obj = ev.get("logo", {})
            img = ""
            if img_obj:
                img = img_obj.get("url", "")
...
if fetch:
    with st.spinner("Ticketmaster ve Eventbrite taranıyor..."):
        ...
        errors = [e for e in [tm_err, eb_err] if e]
        if errors:
            for err in errors:
                st.warning("⚠️ " + err)
    ...
    for i, ev in enumerate(filtered):
        ...
        with cols[i % 3]:
            ...
            st.markdown(
                ...
                + "</div>"
                + ("<div style='color:#00e5a0;font-size:12px;font-family:monospace;text-align:right;margin-top:-8px;margin-bottom:4px'>" + ev["price"] + "</div>" if ev["price"] else "")
                + "<div style='font-size:15px;font-weight:700;color:#fff;margin-bottom:6px;line-height:1.3'>" + ev["name"] + "</div>"
